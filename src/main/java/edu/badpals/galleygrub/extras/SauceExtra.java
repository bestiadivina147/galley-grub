...
public class SauceExtra extends Extra {
    ...
    public void sumExtras(Comanda order) {
        ...
        Optional<Double> sauceCharge = order.itemList().stream().filter(item -> item.extra().equalsIgnoreCase(Extras.SAUCE.name()))
                            .map(item -> Extras.SAUCE.getPrice()).reduce(Double::sum);
